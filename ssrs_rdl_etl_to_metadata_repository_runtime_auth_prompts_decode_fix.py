...
def decode_binary_text(value):
    ...
    Important:
    Some SSRS DataSource image fields are plain ANSI/UTF-8 bytes, while others
    may be UTF-16. If we try UTF-16 first on ANSI bytes, Python can return
    unreadable Chinese-looking characters instead of raising an error.

    This function tries multiple encodings and chooses the most readable result.
    """
    if value is None:
        return None
    ...
    # Remove common BOMs before scoring, but still allow UTF-16 decoding below.
    candidates = []

    # Try single-byte encodings first. This prevents ASCII connection strings
    # from being incorrectly decoded as UTF-16 and displayed as Chinese symbols.
    ...
    keywords = [
        ...
        "sql", "excel", "dsn", "host", "port", "service name"
    ]
    ...
    def score_text(s):
        ...
        # Reward common connection-string patterns.
        for kw in keywords:
            if kw in lowered:
                score += 50

        score += lowered.count("=") * 20
        score += lowered.count(";") * 5

        # Reward normal printable ASCII text.
        printable_ascii = sum(1 for ch in s if 32 <= ord(ch) <= 126)
        score += printable_ascii

        # Penalize replacement/unreadable/control/non-ASCII-heavy output.
        control_chars = sum(1 for ch in s if ord(ch) < 32 and ch not in "\r\n\t")
        non_ascii = sum(1 for ch in s if ord(ch) > 126)
        score -= control_chars * 20
        score -= non_ascii * 2
