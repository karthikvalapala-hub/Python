...
PROCESS_NAME = "SSRS_RDL_ETL_Type2_Report_Load"
...
def fetch_catalog_rows(source_conn):
    """Fetch only SSRS report definitions from dbo.Catalog (Type = 2)."""
    sql = f"""
        SELECT
            ItemID,
            Name,
    ...
    print("Reading Type = 2 report rows from dbo.Catalog...")
    cursor.execute(sql)
...
def main():
    ...
    try:
        print("TYPE 2 REPORT ETL started")
        ...
        for source in SOURCES:
            ...
            try:
                ...
                # PRIMARY EXTRACTION: dbo.Catalog.Type = 2 only.
                report_rows = fetch_catalog_rows(source_conn)

                if not report_rows:
                    print("No Catalog Type = 2 reports found. Nothing to load.")
                    continue

                # This lookup is used only to resolve a Type 2 report's shared
                # datasource reference to its connection details. It does NOT
                # independently load all Type 5 rows in this report script.
                shared_datasource_lookup = fetch_shared_datasource_lookup(source_conn)

                for row in report_rows:
                    try:
                        counts = process_report(
                            cursor,
                            row,
                            shared_datasource_lookup,
        ...
        print("Type 2 Report ETL completed.")
