...
def resolve_connection_username(parsed_username, encrypted_username_value):
    """Return actual username when readable, [ENCRYPTED] when SSRS stores encrypted credential data."""
    if parsed_username:
        return parsed_username
    if encrypted_username_value is not None:
        return "[ENCRYPTED]"
    return None
...
def parse_shared_datasource_content(content_bytes):
    ...
    # Oracle connection strings often contain only DATA SOURCE=<alias/service>
    # and do not have Initial Catalog/Database. Use server_instance as database_name
    # for Oracle when database_name is missing.
    if (
...
def fetch_shared_datasource_lookup(source_conn):
    ...
    sql = f"""
        SELECT
            ...
            CONVERT(VARBINARY(MAX), ds.UserName) AS EncryptedUserName,
    ...
    for row in rows:
        ...
        username = resolve_connection_username(
            parsed.get("connection_username"),
...
def process_report(cursor, source_row, shared_datasource_lookup, rpt_datasource_columns):
    ...
    for q in queries:
        ...
        if datasource_id is None:
            raise ValueError(
                f"Datasource could not be resolved for report '{report_name}', "
                f"dataset '{q.get('dataset_name')}', "
                f"datasource name '{q.get('datasource_name')}'. "
                f"Available datasources: {list(datasource_id_map.keys())}"
            )

        print(
            f"  Dataset datasource resolution: "
            f"Dataset={q.get('dataset_name')}, "
            f"DataSourceName={q.get('datasource_name')}, "
            f"ResolvedDataSourceID={datasource_id}"
        )
