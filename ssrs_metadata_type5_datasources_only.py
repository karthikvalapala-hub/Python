...
PROCESS_NAME = "SSRS_RDL_ETL_Type5_DataSource_Load"
...
def fetch_catalog_rows(source_conn):
    """Fetch only SSRS shared datasource definitions from dbo.Catalog (Type = 5)."""
    sql = f"""
        SELECT
            ItemID,
            Name,
    ...
    print("Reading Type = 5 datasource rows from dbo.Catalog...")
    cursor.execute(sql)
    rows = cursor.fetchall()
    print(f"Type 5 datasource rows fetched: {len(rows)}")
    return rows
...
def fetch_shared_datasource_lookup(source_conn):
    ...
    sql = f"""
        ...
        INNER JOIN {SOURCE_TABLE} report_cat
            ON ds.ItemID = report_cat.ItemID
           AND report_cat.Type = 2
...
def load_catalog_type5_datasources(cursor, type5_datasources, rpt_datasource_columns, source_database):
    ...
    for ds in type5_datasources:
        datasource_id = get_or_create_datasource(
            cursor, ds, rpt_datasource_columns, source_database
        )
        processed_count += 1
        print(
...
def build_type5_datasources_from_catalog(type5_rows):
    """Convert every dbo.Catalog.Type = 5 row into a datasource metadata dictionary."""
    datasources = []

    for row in type5_rows:
        parsed = parse_shared_datasource_content(row.Content)

        datasources.append({
            "datasource_name": row.Name.upper() if row.Name else None,
            "report_datasource_name": None,
            "shared_datasource_path": row.Path,
            "shared_datasource_item_id": str(row.ItemID).upper() if row.ItemID else None,
            "connection_string": (
                parsed.get("connection_string").upper()
                if parsed.get("connection_string") else None
            ),
            "connection_username": parsed.get("connection_username"),
            "server_instance": (
                parsed.get("server_instance").upper()
                if parsed.get("server_instance") else None
            ),
            "database_name": (
                parsed.get("database_name").upper()
                if parsed.get("database_name") else None
            ),
            "datasource_type": parsed.get("datasource_type"),
...
def normalize_type5_datasource_values(cursor):
    """Normalize PROD datasource values without updating report/object tables."""
    update_sql = f"""
        UPDATE {TARGET_RPT_DATASOURCES}
        SET
            Connection_String_Text =
                CASE
                    WHEN Connection_String_Text LIKE '%DATA SOURCE=TSTIJ%'
                    THEN REPLACE(Connection_String_Text, 'DATA SOURCE=TSTIJ', 'DATA SOURCE=PRDIJ')
                    WHEN Connection_String_Text LIKE '%DATA SOURCE=STGIJ%'
                    THEN REPLACE(Connection_String_Text, 'DATA SOURCE=STGIJ', 'DATA SOURCE=PRDIJ')
                    ELSE Connection_String_Text
                END,
            Server_Instance =
                CASE
                    WHEN Server_Instance LIKE 'TSTIJ%' OR Server_Instance LIKE 'STGIJ%'
                    THEN STUFF(Server_Instance, 1, 5, 'PRDIJ')
                    ELSE Server_Instance
                END,
            Database_Name =
                CASE
                    WHEN Database_Name LIKE 'TSTIJ%' OR Database_Name LIKE 'STGIJ%'
    ...
    print(f"RPT_DataSources environment values updated. Rows affected: {cursor.rowcount}")
...
def main():
    ...
    try:
        print("TYPE 5 DATASOURCE ETL started")
        ...
        for source in SOURCES:
            ...
            try:
                ...
                # Verify/extract only Catalog.Type = 5 rows.
                type5_rows = fetch_catalog_rows(source_conn)
                if not type5_rows:
                    print("No Catalog Type = 5 shared datasources found. Nothing to load.")
                    continue

                # Parse and load every dbo.Catalog.Type = 5 row directly.
                # This script does not require a dbo.DataSource join to qualify the row.
                catalog_type5_datasources = build_type5_datasources_from_catalog(type5_rows)

                if catalog_type5_datasources:
                    loaded_count = load_catalog_type5_datasources(
                        cursor,
                        catalog_type5_datasources,
                        rpt_datasource_columns,
                        SOURCE_DATABASE
                    )
                    target_conn.commit()
                    total_datasources += loaded_count
                    print(f"Catalog Type 5 datasource rows processed: {loaded_count}")
                else:
                    print("No matching Type 5 datasource rows were returned from dbo.DataSource join.")

            except Exception as ex:
                if target_conn:
                    target_conn.rollback()
                print("Type 5 datasource load failed for this source.")
                print_error(ex)
                log_error_to_table(
                    target_conn=target_conn,
                    process_name=PROCESS_NAME,
                    source_table=f"{SOURCE_TABLE}, {SOURCE_DATASOURCE_TABLE}",
        ...
        if environment_name.upper() == "PROD":
            try:
                print("\nNormalizing datasource TST/STG values to PRD...")
            ...
            print(f"\nEnvironment is {environment_name}. TST/STG to PRD normalization skipped.")

        print("\nTYPE 5 DATASOURCE LOAD SUMMARY")
        print("-" * 60)
        print(f"Datasource rows processed       : {total_datasources}")
        print("-" * 60)
        print(f"End time: {datetime.now()}")
        print("Type 5 DataSource ETL completed.")
