# ssrs_rdl_etl_to_metadata_repository_errorlog_aligned.py
...
def extract_schema_objects(command_text):
    ...
    patterns = [
        ...
        r"\bexec(?:ute)?\s+([A-Za-z_][\w]*)\.([A-Za-z_][\w]*)",
        r"\bmerge\s+([A-Za-z_][\w]*)\.([A-Za-z_][\w]*)"
    ]

    found = set()
    for pattern in patterns:
        matches = re.findall(pattern, command_text, flags=re.IGNORECASE)
...
def process_report(cursor, source_row):
    ...
    for ds in datasources:
        ...
        if datasource_id is None:
            raise ValueError(f"Failed to create/find datasource for report {report_name}, datasource {datasource_name}")
    ...
    for q in queries:
        ...
        # If the query has no DataSourceName but the report has exactly one datasource, use it.
        ...
        # Some RDLs have datasets with missing/blank DataSourceName, or reference a datasource
        # that is not present in the DataSources section. RPT_Queries.DataSource_ID is NOT NULL,
        # so create/use a placeholder datasource instead of inserting NULL or rolling back the report.
        if datasource_id is None:
            fallback_name = datasource_name or f"UNRESOLVED_DATASOURCE_{report_name}_{q.get('dataset_name')}"
            fallback_ds = {
                "datasource_name": fallback_name,
                "connection_string": None,
                "connection_username": None,
                "server_instance": None,
                "database_name": None,
            ...
            datasource_id = get_or_create_datasource(cursor, fallback_ds)
            datasource_id_map[fallback_name] = datasource_id
            datasource_info_map[fallback_name] = fallback_ds
            datasource_info = fallback_ds
            datasource_count += 1
            ensure_datasource_map(cursor, rpt_id, datasource_id)
            map_count += 1
            print(f"  WARNING: Datasource missing for Dataset = {q.get('dataset_name')}. Used placeholder datasource: {fallback_name} -> DataSource_ID = {datasource_id}")

        if datasource_id is None:
            raise ValueError(f"Datasource_ID is still NULL for dataset: {q.get('dataset_name')}")
