...
# - Binary/image fields in dbo.DataSource are decoded safely and unreadable encrypted bytes are skipped
...
def _to_bytes(value):
    """Return raw bytes for SQL Server image/varbinary values returned by pyodbc."""
    if value is None:
        return None
...
def decode_binary_text(value):
    """Decode SSRS dbo.DataSource image/varbinary fields into readable text.

    Important fix:
    SQL Server ReportServer.dbo.DataSource image fields may contain either
    readable text bytes or protected/encrypted binary bytes. Encrypted binary
    bytes can accidentally decode as Chinese/Japanese-looking characters.
    This function rejects decoded text when it is not mostly readable ASCII
    connection-string/user-name text. That prevents garbage from being loaded
    into RPT_DataSources.Connection_String_Text and Connection_UserName.
    """
    if value is None:
        return None
    ...
    candidates = []
    for enc in ["utf-8-sig", "utf-8", "cp1252", "latin-1", "utf-16", "utf-16-le", "utf-16-be"]:
        try:
    ...
    def score_text(s):
        ...
        lowered = s.lower()
        keywords = [
            "data source", "server", "initial catalog", "database", "provider",
            "user id", "uid", "password", "pwd", "unicode", "oracle", "db2",
            "sql", "excel", "dsn", "host", "port", "service name", "integrated security"
        ]
        ...
        score += lowered.count("=") * 40
        score += lowered.count(";") * 10
        score += sum(1 for ch in s if 32 <= ord(ch) <= 126)

        # Strongly penalize non-ASCII and control characters.
        score -= sum(1 for ch in s if ord(ch) > 126) * 20
        score -= sum(1 for ch in s if ord(ch) < 32 and ch not in "\r\n\t") * 50
        return score

    best_encoding, best_text = max(candidates, key=lambda item: score_text(item[1]))

    # Final guard: do not return unreadable binary/encrypted data.
    if not is_readable_text(best_text):
        return None
...
def is_readable_text(text):
    """Return True only for normal readable datasource text.

    This blocks accidental Chinese-looking output caused by decoding encrypted
    binary bytes with UTF-16/Latin encodings.
    """
    if not text:
        return False
    ...
    # Normal connection strings/usernames are usually short-to-medium and mostly ASCII.
    printable_ascii = sum(1 for ch in text if 32 <= ord(ch) <= 126)
    ...
    ascii_ratio = printable_ascii / total if total else 0

    # Reject mostly non-ASCII text. This is the symptom you are seeing.
    if ascii_ratio < 0.85 or non_ascii > 0:
        return False

    # Reject long binary-looking text with no connection-string/user-name characters.
    useful_chars = sum(1 for ch in text if ch.isalnum() or ch in " _-.\\/;:=@,()[]{}+$%")
    if useful_chars / total < 0.90:
        return False
...
def extract_connect_property(connect_string, property_names):
    """Extract a property from a connection string.

    SSRS dbo.DataSource fields like ConnectionString can be returned by pyodbc
    as bytes, bytearray, or memoryview because the SQL Server column type is image.
    Regex requires a string, so decode before searching.
    """
    connect_string = decode_binary_text(connect_string)
...
def normalize_datasource_type(data_provider, connect_string=None):
    data_provider = decode_binary_text(data_provider)
    connect_string = decode_binary_text(connect_string)
...
def parse_shared_datasource(row):
    # First try Catalog.Content XML for shared data source definitions.
    # This gives clean values when the Type=5 item stores a readable .rds XML body.
    xml_text = decode_xml_content(getattr(row, "Content", None))
    if xml_text:
        try:
            xml_result = parse_shared_datasource_xml(xml_text, row.Name, row.Path)
            if xml_result and (xml_result.get("connection_string") or xml_result.get("data_provider")):
                return xml_result
        except Exception:
            # Continue to dbo.DataSource fallback below.
            pass

    # Fallback source = dbo.DataSource. Some fields are image/varbinary and may be
    # encrypted/protected. decode_binary_text returns None instead of garbage.
    ...
    data_provider = decode_binary_text(getattr(row, "Extension", None))
    datasource_type = normalize_datasource_type(data_provider, connect_string)
